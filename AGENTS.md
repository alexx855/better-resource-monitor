...
- Prefer `Cargo.toml`, `tauri*.json`, scripts, and source over prose docs. If prose guidance drifts, trust the executable configuration in the repo: the app build uses feature `app-store` and the current macOS `minimumSystemVersion` is `13.0`.
